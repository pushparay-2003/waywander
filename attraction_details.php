...
if (isset($_GET['action']) && $_GET['action'] == 'add_wishlist' && isset($_SESSION['user_id'])) {
    ...
    $check_sql = "SELECT * FROM wishlists WHERE user_id = ? AND item_type = 'attraction' AND item_id = ?";
    ...
    if ($check_result->num_rows > 0) {
        header("Location: my_wishlist.php?msg=Already+in+wishlist");
        exit;
    } else {
        $insert_sql = "INSERT INTO wishlists (user_id, item_type, item_id) VALUES (?, 'attraction', ?)";
        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param("ii", $user_id, $attraction_id);
        $insert_stmt->execute();

        header("Location: my_wishlist.php?msg=Added+to+wishlist");
        exit;
    }
}
...
if (isset($_POST['submit_review']) && isset($_SESSION['user_id'])) {
    ...
    if ($rating > 0 && !empty($comment)) {
        $insert_review = "INSERT INTO reviews (user_id, item_type, item_id, rating, comment, created_at) 
                          VALUES (?, 'attraction', ?, ?, ?, NOW())";
        $stmt_review = $conn->prepare($insert_review);
        $stmt_review->bind_param("iiis", $user_id, $attraction_id, $rating, $comment);
        $stmt_review->execute();
    }
    header("Location: attraction_details.php?id=" . $attraction_id);
    exit;
}
...
$reviews_sql = "SELECT r.*, u.fullname 
                ...
                WHERE r.item_type = 'attraction' AND r.item_id = ?
                ORDER BY r.created_at DESC";
$stmt_reviews = $conn->prepare($reviews_sql);
$stmt_reviews->bind_param("i", $attraction_id);
...
<head>
    ...
    <title><?php echo htmlspecialchars($attraction['name']); ?></title>
    ...
    <style>
        ...
        .hero-section {
            position: relative;
            height: 60vh;
            background: url('<?php echo htmlspecialchars($attraction['image_url']); ?>') center/cover fixed no-repeat;
            display: flex;
            align-items: center;
...
<section class="hero-section" data-aos="fade-down" data-aos-duration="1500">
    <div class="hero-overlay">
        ...
        <p><?php echo htmlspecialchars($attraction['rating']); ?>/5 Rating</p>
    </div>
</section>
...
<div class="container py-5" data-aos="fade-up" data-aos-duration="1200">
    ...
    <div class="info-cards" data-aos="fade-up" data-aos-delay="300">
        <div class="info-card">
            <h4>Quick Facts</h4>
            <p><strong>Location:</strong> <?php echo htmlspecialchars($attraction['location'] ?? 'N/A'); ?></p>
            <p><strong>Entry Fee:</strong> <?php echo htmlspecialchars($attraction['entry_fee'] ?? 'Free'); ?></p>
            <p><strong>Opening Hours:</strong> <?php echo htmlspecialchars($attraction['opening_hours'] ?? '9:00 AM - 6:00 PM'); ?></p>
        </div>
        <div class="info-card">
            <h4>Weather</h4>
    ...
    <?php if (!empty($attraction['website_url'])): ?>
        <a href="<?php echo htmlspecialchars($attraction['website_url']); ?>" target="_blank" rel="noopener noreferrer" class="btn btn-info me-3 mb-3" data-aos="zoom-in" data-aos-delay="100">
            Visit Website
        </a>
    <?php endif; ?>

    <?php if (isset($_SESSION['user_id'])): ?>
        <a href="attraction_details.php?id=<?php echo $attraction_id; ?>&action=add_wishlist" class="btn btn-warning wishlist-btn mb-3" data-aos="zoom-in" data-aos-delay="200">Add to Wishlist</a>
    <?php else: ?>
        <a href="login.php" class="btn btn-warning wishlist-btn mb-3" data-aos="zoom-in" data-aos-delay="200">Login to Add to Wishlist</a>
    <?php endif; ?>
